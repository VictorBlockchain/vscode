...
`;
        ...
        vscode.commands.registerCommand('jordi.configureLocalLLM', async () => {
            ...
            if (setupChoice.label.includes('API Keys')) {
                ...
                vscode.window.showInformationMessage('🔑 API Keys Settings Opened!\n\nAdd your API keys in the settings panel. Recommended: DeepSeek (most affordable)', 'Get DeepSeek Key').then(selection => {
                ...
                try {
                    ...
                    if (models.length > 0) {
                        ...
                        if (selectedModel) {
                            ...
                            vscode.window.showInformationMessage(`✅ Ollama configured with ${selectedModel.label}!`, 'Test Connection').then(selection => {
                        ...
                        vscode.window.showInformationMessage('📥 No Ollama models found. Install a recommended model?', 'Install deepseek-coder', 'Install codellama', 'Manual Setup').then(selection => {
                            if (selection === 'Install deepseek-coder') {
                                vscode.window.showInformationMessage('🚀 Run this command in your terminal:\n\nollama pull deepseek-coder\n\nThen try configuring again!', 'Copy Command').then(copySelection => {
                    ...
                    vscode.window.showInformationMessage('🤖 Ollama not detected. Would you like to install it?', 'Install Ollama', 'Manual Setup').then(selection => {
