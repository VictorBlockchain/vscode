...
const chatConfigHandler_1 = require("../utils/chatConfigHandler");
class ChatWebviewProvider {
    constructor(context, aiAgent) {
        this.messages = [];
        this.context = context;
        this.aiAgent = aiAgent;
        this.configHandler = new chatConfigHandler_1.ChatConfigHandler(aiAgent);
    }
    show() {
        if (this.panel) {
            this.panel.reveal();
        ...
        this.panel = vscode.window.createWebviewPanel('web3-ai-chat', 'Web3 AI Agent Chat', vscode.ViewColumn.Beside, {
            enableScripts: true,
...
What would you like to work on today?`,
    ...
    async handleComponentGeneration(input) {
    ...
    async handleContractGeneration(input) {
    ...
    async handleProjectAnalysis(input) {
    ...
    async handleCodeOptimization(input) {
    ...
    async handleTestingQuestions(input) {
    ...
    async handleDeploymentQuestions(input) {
    ...
    async handleSecurityQuestions(input) {
    ...
    async handleGeneralQuestion(input) {
    ...
    async exportChat() {
        const chatContent = this.messages.map(msg => `**${msg.role.toUpperCase()}** (${msg.timestamp.toLocaleString()})\n${msg.content}\n`).join('\n---\n\n');
