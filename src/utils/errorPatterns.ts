...
export function extractErrorDetails(errorMessage: string, pattern: ErrorPattern): { [key: string]: string } {
    const match = errorMessage.match(pattern.pattern);
    const details: { [key: string]: string } = {};
    
    if (match) {
        // Extract common placeholders
        if (match[1]) details.module = match[1];
        if (match[2]) details.type = match[2];
        if (match[1] && pattern.pattern.source.includes('Port')) details.port = match[1];
    }
    
    return details;
}

export function generateFixCommands(pattern: ErrorPattern, details: { [key: string]: string }): string[] {
