...
async function showWelcomeExperience(context: vscode.ExtensionContext, chatProvider: ChatWebviewProvider) {
    ...
    if (isFirstTime) {
        ...
        if (!hasWorkspace) {
            ...
            const action = await vscode.window.showInformationMessage(
                '🤖 Welcome to Jordi AI Agent! To get started, please open a folder or workspace.',
                'Open Folder',
                'Open Workspace',
        ...
        const action = await vscode.window.showInformationMessage(
            '🤖 Jordi AI Agent is ready! Your smart coding companion for Next.js and Web3 development.',
            'Open Chat',
            'Show Memory'
        );
...
function showGettingStartedMessage(chatProvider: ChatWebviewProvider) {
    const action = vscode.window.showInformationMessage(
        '🎉 Jordi is ready to help! I can assist with Next.js development, Web3 projects (Solana & Sui), code optimization, and much more.',
        'Start Chatting',
        'View Features',
        'Configure AI'
    ).then(selection => {
...
function showFeaturesOverview() {
    const panel = vscode.window.createWebviewPanel(
        'jordiFeaturesOverview',
        '🤖 Jordi AI Agent - Features',
        vscode.ViewColumn.One,
        {
            enableScripts: true,
            retainContextWhenHidden: true
